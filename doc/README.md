# 📚 Tài Liệu Dự Án — Chatbot Quản Lý Sự Kiện & Nhắc Việc Trên Mezon

Chào mừng bạn đến với tài liệu đầy đủ của hệ thống **Chatbot hỗ trợ quản lý sự kiện và nhắc việc tự động trên nền tảng Mezon** (đề tài thực tập tốt nghiệp — Khoa CNTT, Trường ĐH Quy Nhơn).

## 📋 Mục Lục Tài Liệu

### 🏗️ Kiến Trúc & Thiết Kế
- [**Tổng Quan Hệ Thống**](./system-overview.md) - Kiến trúc tổng thể và luồng hoạt động
- [**Database Schema**](./database-schema.md) - Cấu trúc cơ sở dữ liệu chi tiết
- [**API Reference**](./api-reference.md) - Tài liệu API và endpoints

### 🤖 Bot Commands
- [**Hướng Dẫn Sử Dụng**](./user-guide.md) - Hướng dẫn đầy đủ cho người dùng cuối
- [**Command Reference**](./command-reference.md) - Tài liệu chi tiết tất cả lệnh bot
- [**Interactive Features**](./interactive-features.md) - Forms, buttons và tương tác

### 💻 Development
- [**Setup & Installation**](./setup-installation.md) - Hướng dẫn cài đặt và cấu hình
- [**Development Guide**](./development-guide.md) - Quy trình phát triển và coding standards
- [**Deployment Guide**](./deployment-guide.md) - Hướng dẫn deploy production

### 🔧 Tính Năng Nâng Cao
- [**Recurring Events**](./recurring-events.md) - Hệ thống lịch lặp lại
- [**Reminder System**](./reminder-system.md) - Hệ thống nhắc nhở tự động
- [**Priority & Tags**](./priority-tags.md) - Quản lý ưu tiên và nhãn
- [**Admin Guide**](./admin-guide.md) - Hệ thống quản trị (AdminGuard, broadcast, user management)
- [**Mezon Landing Design**](./mezon-landing-design.md) - Landing page design specification

### 🛠️ Maintenance & Tiến Độ
- [**Troubleshooting**](./troubleshooting.md) - Xử lý sự cố thường gặp
- [**Changelog**](./CHANGELOG.md) - Lịch sử thay đổi và phiên bản
- [**Progress**](./PROGRESS.md) - Tiến độ implementation (design vs code)

---

## 🚀 Quick Start

Nếu bạn là người mới, hãy bắt đầu với:

1. **Người dùng cuối**: Đọc [Hướng Dẫn Sử Dụng](./user-guide.md)
2. **Developer**: Đọc [Setup & Installation](./setup-installation.md) → [Development Guide](./development-guide.md)
3. **DevOps**: Đọc [Deployment Guide](./deployment-guide.md) → [Troubleshooting](./troubleshooting.md)

---

## 📝 Cập Nhật Tài Liệu

Tài liệu này được cập nhật thường xuyên. Khi thêm tính năng mới hoặc thay đổi hệ thống, vui lòng:

1. Cập nhật tài liệu tương ứng
2. Thêm entry vào changelog
3. Review và merge cùng với code changes

---

## 🤝 Đóng Góp

Nếu bạn tìm thấy lỗi trong tài liệu hoặc muốn cải thiện, vui lòng:

1. Tạo issue mô tả vấn đề
2. Hoặc tạo PR với các thay đổi đề xuất
3. Follow coding standards và documentation guidelines

---

**Phiên bản tài liệu**: v1.1  
**Cập nhật lần cuối**: 14/05/2026  
**Tương thích với**: Bot v0.1.0 + Web v0.1.0