...
import { SCHEDULE_STATUSES } from '../schedules/entities/schedule.entity';
...
export class AdminController {
  ...
  async setRole(
    ...
    if (!role || !USER_ROLES.includes(role as UserRole)) {
      throw new BadRequestException(`role phải là một trong: ${USER_ROLES.join(', ')}`);
    }
  ...
  @Delete('users/:userId')
  async deleteUser(@Param('userId') userId: string, @Req() req: AuthenticatedRequest) {
  ...
  async listSchedules(
    ...
    if (status && !(SCHEDULE_STATUSES as readonly string[]).includes(status)) {
      throw new BadRequestException(`status phải là một trong: ${SCHEDULE_STATUSES.join(', ')}`);
    }
  ...
  async listAudit(
    ...
    const result = await this.adminService.listAuditLogs({
      ...
      schedule_id: scheduleId ? parseInteger(scheduleId, Number.NaN) : undefined,
    });
    return { success: true, ...result };
  }
  ...
  @Get('broadcasts')
  async listBroadcasts(@Query('page') page?: string, @Query('limit') limit?: string) {
  ...
  async setSetting(
    ...
    const setting = await this.adminService.setSetting(key, value, req.session.sub);
