...
export const mockSchedules: Schedule[] = [
  ...
  },
  {
    ...
    description:
      'Thống nhất các chỉ số KPI trong tâm cho quý cuối cùng của năm. Xem xét báo cáo doanh thu từ các chi nhánh miền Bắc và miền Nam. Thảo luận về lộ trình chuyển đổi số toàn diện cho hệ thống FocusFlow Pro trong năm 2024.',
    type: 'hop-hanh',
...
export const schedulePriorityOptions = [
  { value: 'high', label: 'Cao' },
  { value: 'normal', label: 'Trung bình' },
  { value: 'low', label: 'Thấp' },
] as const;

export type ApiScheduleItemType = (typeof scheduleTypeOptions)[number]['value'];
export type ApiSchedulePriority = (typeof schedulePriorityOptions)[number]['value'];

export function toApiScheduleItemType(value?: string | null): ApiScheduleItemType {
  if (value === 'task' || value === 'meeting' || value === 'event' || value === 'reminder') {
    return value;
  }
  if (value === 'hop-hanh') return 'meeting';
  if (value === 'ca-nhan') return 'event';
  return 'task';
}

export function toApiSchedulePriority(value?: string | null): ApiSchedulePriority {
  if (value === 'high' || value === 'normal' || value === 'low') {
    return value;
  }
  if (value === 'cao') return 'high';
  if (value === 'thap') return 'low';
  return 'normal';
}
