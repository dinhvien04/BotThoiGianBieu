...
describe('AuthService', () => {
  ...
  it('rejects invalid Mezon channel hash signatures', async () => {
    const rawHashData = createChannelHashData(appSecret, now).replace('mezon_dev', 'attacker');

    await expect(service.authenticateChannelApp({ rawHashData })).rejects.toThrow(
      'Invalid Mezon hash signature',
    );
  ...
  it('reads quoted cookie values', async () => {
    const rawHashData = createChannelHashData(appSecret, now);
    const result = await service.authenticateChannelApp({ rawHashData });
    ...
    expect(session?.sub).toBe('123456789');
  });

  it('forces Secure when SameSite=None is configured', () => {
    const serviceWithNoneCookie = makeService({ AUTH_COOKIE_SAMESITE: 'None' }, usersService);

    const cookie = serviceWithNoneCookie.createSessionCookie('token');
  ...
  it('requires a dedicated session secret in production', async () => {
    ...
    await expect(
      serviceWithoutSessionSecret.authenticateChannelApp({ rawHashData }),
    ).rejects.toThrow('AUTH_TOKEN_SECRET is required in production');
  });

  it('creates a 128-bit base64url OAuth state', () => {
    const serviceWithOAuthConfig = makeService(
      {
        AUTH_TOKEN_SECRET: sessionSecret,
        MEZON_CLIENT_ID: 'client-id',
        MEZON_CLIENT_SECRET: 'client-secret',
      },
      usersService,
    );

    const start = serviceWithOAuthConfig.createMezonOAuthStart('/dashboard');
    const stateCookie = start.cookies.find((cookie) => cookie.startsWith('btgb_mezon_state='));
    const state = decodeURIComponent(stateCookie?.match(/^btgb_mezon_state=([^;]+)/)?.[1] ?? '');
...
function makeService(
  ...
  usersService: jest.Mocked<Pick<UsersService, 'registerUser'>>,
): AuthService {
  const config = {
    get: jest.fn((key: string) => values[key]),
  } as unknown as ConfigService;
  return new AuthService(config, usersService as unknown as UsersService);
}
...
function createChannelHashData(secret: string, authDate: number): string {
  ...
  const hash = createHmac('sha256', secretKey).update(queryData).digest('hex');
