...
describe('AdminService', () => {
  ...
  describe('listUsers', () => {
    it('applies search/role/locked filters and pagination', async () => {
      ...
      userRepo.createQueryBuilder.mockReturnValueOnce(qb).mockReturnValueOnce(countQb);
      ...
      expect(qb.groupBy).toHaveBeenCalledWith(expect.stringContaining('u.username'));
      expect(qb.groupBy).toHaveBeenCalledWith(expect.stringContaining('u.updated_at'));
  ...
  describe('user mutations', () => {
    ...
    it('setRole throws when user missing', async () => {
      ...
      await expect(service.setRole('nope', 'admin')).rejects.toBeInstanceOf(NotFoundException);
    });

    it('setLocked updates is_locked', async () => {
      const user = { user_id: 'u-1', is_locked: false, token_version: 0 } as User;
      userRepo.findOne.mockResolvedValue(user);
    ...
    it('deleteUser cascades to schedules/audit/settings', async () => {
      ...
      expect(txQueryFn).toHaveBeenCalledWith(expect.stringContaining('schedule_audit_logs'), [
        'u-1',
      ]);
  ...
  describe('listSchedules', () => {
    ...
    it('deleteSchedule errors when not found', async () => {
      ...
      await expect(service.deleteSchedule(99)).rejects.toBeInstanceOf(NotFoundException);
    });
