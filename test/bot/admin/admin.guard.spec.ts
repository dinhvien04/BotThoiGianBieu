...
describe('AdminGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new AdminGuard(authService as never, usersService as never);
  });

  it('throws Unauthorized when session is missing', async () => {
    authService.readSessionFromCookie.mockReturnValue(null);
    await expect(guard.canActivate(makeContext('anything'))).rejects.toBeInstanceOf(
      UnauthorizedException,
    );
  });

  it('throws Forbidden when user not found in DB', async () => {
    authService.readSessionFromCookie.mockReturnValue({ sub: 'u-1', tokenVersion: 0 });
    usersService.findByUserId.mockResolvedValue(null);
    await expect(guard.canActivate(makeContext('c'))).rejects.toBeInstanceOf(ForbiddenException);
  });
  ...
  it('throws Forbidden when user is locked', async () => {
    ...
    await expect(guard.canActivate(makeContext('c'))).rejects.toThrow('Account locked');
  });

  it('throws Unauthorized when token version is stale', async () => {
    authService.readSessionFromCookie.mockReturnValue({ sub: 'u-1', tokenVersion: 1 });
    usersService.findByUserId.mockResolvedValue({
      user_id: 'u-1',
      role: 'admin',
      is_locked: false,
    ...
    await expect(guard.canActivate(makeContext('c'))).rejects.toThrow('Session revoked');
  });
  ...
  it('throws Forbidden when user is not admin', async () => {
    ...
    await expect(guard.canActivate(makeContext('c'))).rejects.toThrow('Admin access required');
  });

  it('allows admin user and stores session on request', async () => {
    const session = { sub: 'admin-1', username: 'boss', tokenVersion: 0 };
  ...
  it('handles array cookie header by taking first element', async () => {
    ...
    await expect(guard.canActivate(ctx)).resolves.toBe(true);
    expect(authService.readSessionFromCookie).toHaveBeenCalledWith('first-cookie');
  });
});
